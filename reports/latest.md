...
- **2026-09-21 · 5812804 · docs(relatorios)** — Atualização de status no `relatorio.html` do cliente: banner "Última entrega" mudou de "em testes finais" para "no ar ✅", badge da entrada de 21 Set passou para a classe `done` ("Entregue") e descrição reescrita registrando a publicação da seção *Trabaja con nosotros* em `hdm-six.vercel.app/trabaja-con-nosotros`. Diff mínimo: 1 arquivo, +3/-3, sem estrutura HTML ou scripts tocados.
- **2026-09-21 · 0471dd2 · docs(documentos/relatorios)** — Sincronização do `relatorio.html` com a versão servida no VPS (declarada autoritativa). Adições: CSS `.back-to-panel` (pill fixa com regra mobile), link "Voltar ao painel" (`<a href="/cliente/">`) e changelog interno atualizado (Trabaja con nosotros com badge "Em testes finais"). Único ponto de atenção: href absoluto depende de `/cliente/` existir no host.
...
- Monitoramento operacional do fluxo de candidaturas recém-publicado: checagem de recebimento de CVs no inbox `rrhh@hdmindustrial.es` e comportamento do `mailto:` em clientes variados (evidência: recomendação registrada na entrada 69b5338 para os primeiros dias após publicação).
...
- Confirmar se a página legal de Política de Privacidade foi de fato atualizada para cobrir dados de candidatos — os dicionários ganharam a cláusula "Datos de candidatos", mas o diff de nenhum dos commits lista o arquivo da política.
- Verificar que o link `/cliente/` responde corretamente no ambiente publicado (botão "Voltar ao painel" do relatório).
- Verificar independentemente o deploy declarado: o commit 5812804 registra a seção "no ar" em `hdm-six.vercel.app/trabaja-con-nosotros`, mas o diff é docs-only e não prova o deploy — checagem de smoke test no ambiente publicado fecha a pendência.
...
- O commit 5812804 é informativo (docs), não funcional: a funcionalidade entregue permanece a dos commits 75e3d08/69b5338; o diff de status não verifica o deploy, apenas o registra.
